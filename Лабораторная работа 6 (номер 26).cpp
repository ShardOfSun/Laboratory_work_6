...
void sort(int**, int, int); // Сортировка массива 1-х эл-в каждой стр-ки и их индексов
...
{	int i, len;
	...
	{	i = arrf[k][1]; // Индекс в arr каждого первого эл-та (по порядку в arrf)
		...
		for (int j = 0; j < len; ++j)
		{	arrr[k][j] = arr[i][j];	}	} /* Каждому эл-ту искомой ставим в соответствие
...
{	// Проходимся по первым эл-м каждой строки начального массива
	for (int i = 0; i < n; ++i)
	{	delete[] arr1[i];
		delete[] arr2[i];
		delete[] arr3[i];	}
